...
class Agent(nn.Module):
    def __init__(self, env, args: Args):
        ...
        self.actor_mean = CPGNetwork(
            [np.prod(env.observation_space.shape), 64, 64],
            1,
            [64, np.prod(env.action_space.shape)],
    ...
    def get_action_and_value(
        self, x, state: torch.Tensor, timestep: float | torch.Tensor, action=None
    ):
...
if __name__ == "__main__":
    ...
    states = torch.zeros((args.num_steps,) + agent.actor_mean.state_shape)
